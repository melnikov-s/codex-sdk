...
function SourcesPanel({ sources }) {
  return h(
    ...
    sources.length === 0
      ? h(Text, { dimColor: true }, "No sources added yet. Paste URLs or mention papers.")
...
export const workflow = createAgentWorkflow(({ setState, state, actions, tools }) => {
  ...
  function updateSlots() {
    setState({
      slots: {
        aboveHeader: h(Text, { bold: true, color: "#a0e7e5" }, "📚 Research Session"),
        belowHeader: h(Text, { dimColor: true }, `Citation style: ${style}`),
        aboveHistory: h(SourcesPanel, { sources }),
        belowHistory: h(Text, { dimColor: true }, "Tip: Use /style apa|mla|chicago to switch citation style"),
        aboveInput: h(Text, { color: "#a8e6cf" }, `Sources: ${sources.length}`),
        belowInput: h(Text, { dimColor: true }, "Shortcuts: /style, /clear-sources"),
      },
    });
  }
  ...
  function extractUserSources(text) {
    ...
    const mentions = Array.from(text.matchAll(/\b(arXiv:\s*\d{4}\.\d{4,5}|doi:\s*\S+)\b/gi)).map(m => m[0]);
  ...
  return {
    ...
    initialize: () => {
      ...
      setState({
        messages: [
          { role: "ui", content: "Paste relevant links or describe your research question." },
        ],
      });
    },
    ...
    terminate: () => {
      setState({
        ...
        slots: { aboveHeader: null, belowHeader: null, aboveHistory: null, belowHistory: null, aboveInput: null, belowInput: null },
      });
      sources = [];
    },
    ...
    commands: {
      style: {
        ...
        handler: (args) => {
          const next = String(args || "").trim().toUpperCase();
