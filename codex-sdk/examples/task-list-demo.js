...
const codeReviewWorkflow = createAgentWorkflow(
  ({ state, setState, actions }) => {
    ...
    const getCodeFiles = () => {
      const files = [];
      const codeExtensions = ['.js', '.ts', '.tsx', '.jsx', '.py', '.go', '.rs', '.java'];
    ...
    return {
      initialize: async () => {
        setState({ messages: [{ role: "ui", content: "🔍 Code Review Assistant - Analyzing codebase..." }] });
        ...
        if (files.length === 0) {
          ...
          actions.addMessage({
            role: "ui",
            content: "No code files found in current directory. Create some .js, .ts, or .py files first!"
          });
          return;
        }
...
Be specific about files and what needs to be done.`,
        ...
        const taskLines = result.text.split('\n').filter(line => 
        ...
        actions.addMessage({
          role: "ui",
          content: `📋 Analysis complete! I found ${taskLines.length} areas for improvement in your codebase.
...
Type "start" to begin working on the first task, or say "help task 1" for guidance on any specific task.`
      ...
      message: async (input) => {
        ...
        if (userMessage === "start") {
          ...
          if (nextTask) {
            actions.addMessage({
              role: "ui", 
              content: `🎯 Let's work on: "${nextTask.label}"\n\nWhen you're done, say "done" to mark it complete, or "help" if you need guidance.`
            });
          } else {
            actions.addMessage({
              role: "ui",
              content: "🎉 All tasks completed! Great work on improving your codebase."
            });
          }
        }
        else if (userMessage === "done" || userMessage.includes("completed")) {
          actions.toggleTask(); // Toggle next incomplete task
          actions.addMessage({
            role: "ui",
            content: "✅ Task marked complete! Type 'start' for the next task."
          });
        }
          ...
          if (nextTask) {
            const result = await generateText({
              model: openai("gpt-4o-mini"),
              system: "Provide specific, actionable steps to fix this code issue. Be concise and helpful.",
              messages: [{
                role: "user",
            ...
            actions.addMessage({
              role: "ui",
              content: "No pending tasks! All issues have been resolved."
            });
          }
        }
        else {
          ...
          const result = await generateText({
            model: openai("gpt-4o-mini"),
            system: "You are a code review assistant. Help the user with their questions about code improvement and development.",
      ...
      stop: () => {
        ...
        actions.addMessage({
          role: "ui",
          content: "Agent paused. Task list preserved.",
        });
      },
      ...
      displayConfig: {
        header: React.createElement(Text, { bold: true, color: "magenta" }, "🔍 Code Review Assistant"),
        formatRoleHeader: (message) => {
          if (message.role === "assistant") {
            return React.createElement(Text, { bold: true, color: "green" }, "🤖 Code Reviewer");
          }
          if (message.role === "user") {
            return React.createElement(Text, { bold: true, color: "cyan" }, "👨‍💻 Developer");
          }
          if (message.role === "ui") {
            return React.createElement(Text, { bold: true, color: "yellow" }, "📋 System");
          }
