  ...
  ({ setState, state, actions, tools }) => {
    ...
    async function runCodebaseAnalysis() {
      try {
        ...
        await simpleFileAnalysis(
          "../README.md",
          "project overview from parent",
        );
        await simpleFileAnalysis("package.json", "project dependencies");
        actions.say(
          "✅ Basic codebase analysis complete! Ready to generate quiz questions...",
        );
        finishAnalysis();
      } catch (error) {
    ...
    function finishAnalysis() {
      ...
      actions.say(
        "✅ Codebase analysis complete! Generating your first quiz question...",
      );
      startQuiz();
    }
    ...
    async function startQuiz() {
      quizActive = true;
      actions.say(
        `📊 Score: ${quizState.score}/${quizState.totalQuestions} | Question: ${quizState.currentQuestion + 1}/${quizState.totalQuestions}`,
      );
...
DO NOT repeat any previous questions. Generate something new about ${currentTopic}.`;
    ...
    function handleQuizAnswer(toolResponse) {
      ...
      if (Array.isArray(toolResponse.content)) {
        const result = toolResponse.content.find(
    ...
    return {
      ...
      message: async (userInput) => {
        ...
        actions.say(
          "📝 I'll consider your input for the current question analysis...",
        );
      },
