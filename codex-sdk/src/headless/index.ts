...
import { createApplyPatchTool, createShellTool } from "../components/chat/tools/definitions.js";
import { execToolCall } from "../tools/runtime.js";
import { ReviewDecision } from "../utils/agent/review.js";
import { getId, getMessageType, getTextContent, getToolCalls } from "../utils/ai.js";
	...
	switch (msg.role) {
		...
		case "tool": {
			if (Array.isArray(msg.content)) {
				const part = msg.content.find((p) => (p as { type?: string } | undefined)?.type === "tool-result") as
					| { toolName?: string }
					| undefined;
...
export function runHeadless(
	...
	const sink = options.log?.sink ?? ((line: string) => process.stdout.write(line + "\n"));
	...
	const printNewMessages = (messages: Array<UIMessage>) => {
		for (const msg of messages) {
			...
			seenIds.add(id);
			const line = mode === "jsonl" ? jsonlFormatLine(msg) : humanFormatLine(msg, roleHeader, messageFmt);
			if (line != null) {
				sink(line);
			}
		}
	...
	const smartSetState: WorkflowHooks["setState"] = async (updater) => {
		...
		if (typeof updater === "function") {
			newState = (updater as (prev: WorkflowState) => WorkflowState)(syncRef.current);
		} else {
			newState = { ...(syncRef.current as object), ...(updater as object) } as WorkflowState;
		}
	...
	const actions: WorkflowHooks["actions"] = {
		...
		setInputDisabled: (disabled: boolean) => void smartSetState({ inputDisabled: disabled }),
		...
		clearSlot: (region) =>
			void smartSetState((prev) => {
				const slots = { ...(prev.slots || {}) } as NonNullable<WorkflowState["slots"]>;
		...
		addToQueue: (item: string | Array<string>) =>
			void smartSetState((prev) => ({ ...prev, queue: appendItems(prev.queue || [], Array.isArray(item) ? item : [item]) })),
		removeFromQueue: () => {
		...
		addTask: (task) =>
			void smartSetState((prev) => ({ ...prev, taskList: coerceTaskItems(task) })),
		...
		clearTaskList: () => void smartSetState((prev) => ({ ...prev, taskList: [] })),
		...
		handleModelResult: async (result, opts) => {
			...
			const toolResponses = (await executeTools(messages as unknown as Array<ModelMessage>, opts)) as Array<ModelMessage>;
	...
	const hooks: WorkflowHooks = {
		...
		control: {
			message: (input: string | ModelMessage) => {
				const normalized: ModelMessage = typeof input === "string" ? { role: "user", content: input } : input;
		...
		approval: {
			...
			setPolicy: (policy: ApprovalPolicy) => void smartSetState({ approvalPolicy: policy }),
			canAutoApprove: async (
				...
				return canAutoApprove(command, workdir, current, writableRoots || additionalWritableRoots);
			},
		},
	} as WorkflowHooks;

	const workflow = (workflowFactory || ((_hooks: WorkflowHooks) => ({
		initialize() {},
		message() {},
		stop() {},
		terminate() {},
	}) as unknown as Workflow))(hooks);
	...
	const controller: WorkflowController = {
		headless: true,
		message: (input: string | ModelMessage) => {
			const normalized: ModelMessage = typeof input === "string"
