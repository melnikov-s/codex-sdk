...
export function coerceTaskItems(
  input: string | TaskItem | Array<string | TaskItem>,
): Array<TaskItem> {
  const list = Array.isArray(input) ? input : [input];
  return list.map((t) =>
    typeof t === "string"
      ? { completed: false, label: t }
      : { completed: t.completed, label: t.label },
  );
}
...
export function toggleNextIncomplete(
  list: ReadonlyArray<TaskItem>,
): Array<TaskItem> {
