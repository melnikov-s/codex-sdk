...
export function useWorkflowManager(params: {
	...
	const { initialApprovalPolicy, additionalWritableRoots, uiConfig, workflowFactory, selectionApi, promptApi, onController } = params;
	...
	const handleToolCall = useToolExecution({
		...
		dispatchUserMessage: (message: ModelMessage) => {
			...
			const normalized: ModelMessage = typeof message === "string" ? { role: "user", content: message } : message;
	...
	useEffect(() => {
		...
		const workflowHooks: WorkflowHooks = {
			...
			prompts: {
				...
				) => selectionApi.openSelection(items as Array<{ label: string; value: string }>, options as { label?: string; timeout?: number; defaultValue: string }),
				confirm: (
					message: string,
					options: ConfirmOptions | ConfirmOptionsWithTimeout,
