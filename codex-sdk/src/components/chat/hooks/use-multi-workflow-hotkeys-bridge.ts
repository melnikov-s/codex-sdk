...
export interface BridgeMultiWorkflowMgr {
  ...
  createInstance?: (factory: unknown, options?: { activate?: boolean; title?: string }) => string | void;
  removeInstance: (
    id: string,
...
export function useMultiWorkflowHotkeysBridge(params: {
  ...
  useMultiWorkflowHotkeys({
    ...
    switchToPreviousWorkflow: () => {
      ...
      const workflows = multiWorkflowMgr.listInstances();
      const currentIndex = workflows.findIndex((w: Instance) => w.isActive);
      const prevIndex = (currentIndex - 1 + workflows.length) % workflows.length;
    ...
    openWorkflowPicker: () => {
      ...
      openSelection(items, {
        defaultValue: multiWorkflowMgr.activeWorkflowId || (items[0]?.value ?? ""),
    ...
    createNewWorkflow: () => {
      if (!isMulti || !availableWorkflows?.length) {return;}
