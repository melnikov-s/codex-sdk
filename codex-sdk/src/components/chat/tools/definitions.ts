...
export function createShellTool() {
  const ShellToolParametersSchema = z.object({
    cmd: z.array(z.string()).describe("The command and its arguments to execute."),
    workdir: z.string().describe("The working directory for the command."),
    timeout: z
      .number()
      .describe("The maximum time to wait for the command to complete in milliseconds."),
  });
...
export function createApplyPatchTool() {
  const ApplyPatchParametersSchema = z.object({
    cmd: z.array(z.string()).describe("The apply_patch invocation and payload."),
    workdir: z.string().describe("The working directory for the command."),
    timeout: z
      .number()
      .describe("The maximum time to wait for the command to complete in milliseconds."),
...
export function createUserSelectTool() {
  const UserSelectSchema = z.object({
    ...
    options: z.array(z.string()).describe("Array of option strings for user to choose from"),
    defaultValue: z
      .string()
      .describe(
